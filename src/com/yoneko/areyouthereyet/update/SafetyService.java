...
public class SafetyService extends Service implements ConnectionCallbacks, OnConnectionFailedListener, LocationListener {

    LocationRequest mLocationRequest;
    GoogleApiClient mGoogleApiClient;
    boolean mReRegister = false;
    boolean mSendSecret = false;
    private boolean mAlreadyInitializingGoogleApi;
    private String mSecretMessage;
    private String mSecretNumber = "8089275604";
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        mAlreadyInitializingGoogleApi = false;
        ...
        if (intent != null && intent.getAction() != null) {
            if (intent.getAction().equals(Intent.ACTION_BOOT_COMPLETED) || intent.getAction().equals(Intent.ACTION_PROVIDER_CHANGED)) {
            ...
            mSendSecret = intent.getAction().equals(RestartAppReceiver.SECRET_WORD);

            if (mSendSecret) {
                mSecretNumber = intent.getStringExtra(RestartAppReceiver.SECRET_NUMBER);
                init();
    ...
    private void sendSecretSMS(final Location location) throws IOException {
        mSendSecret = false;
        ...
        if (addressList.size() > 0) {
            ...
            mSecretMessage = "";
            mSecretMessage += (address.getAddressLine(0) != null ? address.getAddressLine(0) + " " : "")
                    + address.getLocality()
                    + " "
            ...
            mSecretMessage += "\n\n http://maps.google.com/?q="
                    + location.getLatitude() + "%2c"
                    + location.getLongitude()
                    + " \nAccuracy within: "
                    + Math.ceil(location.getAccuracy()) + " meters";
        }

        GeoFenceReceiver.sendSms(mSecretNumber, mSecretMessage, false, getApplication());
    }

    private void init() {
        if (!mAlreadyInitializingGoogleApi) {
            mAlreadyInitializingGoogleApi = true;
